...
public class MainActivity extends Activity {
    ...
    private View buildUi() {
        ...
        TextView help=label("로그인 세션은 서비스별로 이 기기의 WebView에 저장됩니다. 두 서비스의 화면 구조가 바뀌면 일시적으로 사용량을 읽지 못할 수 있습니다.",14,Color.rgb(170,170,170),false); help.setLineSpacing(dp(3),1f); root.addView(help); scroll.addView(root); return scroll;
    }

    private View providerCard(String provider,String name,int accent) {
        ProviderViews v=new ProviderViews(); if (UsageService.CLAUDE.equals(provider)) claudeViews=v; else codexViews=v;
    ...
    private View metric(String name,boolean session,ProviderViews v) {
        ...
        TextView n=label(name,14,Color.rgb(215,215,215),false); line.addView(n,new LinearLayout.LayoutParams(0,-2,1)); TextView value=label("—%",22,Color.WHITE,true); line.addView(value); if(session)v.sessionValue=value;else v.weeklyValue=value;box.addView(line);
    ...
    private void renderProvider(String provider,JSONObject o) { ProviderViews v=UsageService.CLAUDE.equals(provider)?claudeViews:codexViews;if(v==null)return;boolean login=o.optBoolean("needsLogin"),ok=o.optBoolean("ok");v.status.setText(login?"로그인 필요":ok?"정상적으로 확인됨":"확인 중…");v.status.setTextColor(login?Color.rgb(255,185,75):ok?Color.rgb(120,215,160):Color.rgb(217,119,87));apply(o.optJSONObject("session"),v.sessionValue,v.sessionBar,v.sessionReset);apply(o.optJSONObject("weekly"),v.weeklyValue,v.weeklyBar,v.weeklyReset);long at=prefs.getLong("updated_"+provider,0);v.updated.setText(at==0?"마지막 확인: -":"마지막 확인: "+android.text.format.DateFormat.format("M/d HH:mm",at));}
    ...
    private static class ProviderViews{TextView status,updated,sessionValue,weeklyValue,sessionReset,weeklyReset;ProgressBar sessionBar,weeklyBar;}
}
